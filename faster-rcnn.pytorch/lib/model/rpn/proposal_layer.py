...
class _ProposalLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        # anchors = self._anchors.view(1, A, 4) + shifts.view(1, K, 
        # 4).permute(1, 0, 2).contiguous()
        ...
        # proposals_keep = proposals.view(-1, 4)[keep_idx, :].contiguous(
        # ).view(batch_size, trim_size, 4)
        ...
        for i in range(batch_size):
            ...
            # threshold
            # # (NOTE: convert min_size to input image scale stored in 
            # im_info[2])
            proposals_single = proposals_keep[i]
            scores_single = scores_keep[i]

            # # 4. sort all (proposal, score) pairs by score from highest to 
            # lowest
