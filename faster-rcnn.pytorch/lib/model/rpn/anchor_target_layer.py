...
class _AnchorTargetLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        keep = ((all_anchors[:, 0] >= -self._allowed_border) &
                ...
                (all_anchors[:, 2] < long(
                    im_info[0][1]) + self._allowed_border) &
                (all_anchors[:, 3] < long(
                    im_info[0][0]) + self._allowed_border))
        ...
        for i in range(batch_size):
            ...
            if sum_fg[i] > num_fg:
                ...
                # details.
                # use numpy instead.
                # rand_num = torch.randperm(fg_inds.size(0)).type_as(
                # gt_boxes).long()
            ...
            if sum_bg[i] > num_bg:
                ...
                # rand_num = torch.randperm(bg_inds.size(0)).type_as(
                # gt_boxes).long()
        ...
        labels = labels.view(batch_size, height, width, A).permute(0, 3, 1,
                                                                   2).contiguous()
        ...
        bbox_inside_weights = bbox_inside_weights.view(batch_size,
                                                       anchors_count, 1).expand(
            batch_size, anchors_count,
            4)
...
def _unmap(data, count, inds, batch_size, fill=0):
    ...
    if data.dim() == 2:
        ...
        ret = torch.Tensor(batch_size, count, data.size(2)).fill_(fill).type_as(
            data)
        ret[:, inds, :] = data
    return ret
