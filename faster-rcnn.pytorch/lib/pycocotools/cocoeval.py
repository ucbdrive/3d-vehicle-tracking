...
class COCOeval:
    ...
    #  useCats    - [1] if true use category labels for evaluation    # Note:
    #  if useSegm=0 the evaluation is run on
    #  bounding boxes.
    ...
    def _prepare(self):
        ...
        def _toMask(objs, coco):
            ...
            for obj in objs:
                ...
                elif type(obj['segmentation']) == dict and type(
                        obj['segmentation']['counts']) == list:
                    obj['segmentation'] = \
                        mask.frPyObjects([obj['segmentation']], t['height'],
                                         t['width'])[0]
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        gtind = [ind for (ind, g) in
                 sorted(enumerate(gt), key=lambda ind_g: ind_g[1]['_ignore'])]
        ...
        ious = self.ious[imgId, catId][0:maxDet,
               np.array(gtind)] if N_iou > 0 else self.ious[imgId, catId]
    ...
    def summarize(self):
        ...
        def _summarize(ap=1, iouThr=None, areaRng='all', maxDets=100):
            ...
            iouStr = '%0.2f:%0.2f' % (
                p.iouThrs[0], p.iouThrs[-1]) if iouThr is None else '%0.2f' % (
                iouThr)
            areaStr = areaRng
            maxDetsStr = '%d' % (maxDets)

            aind = [i for i, aRng in
                    enumerate(['all', 'small', 'medium', 'large']) if
                    aRng == areaRng]
