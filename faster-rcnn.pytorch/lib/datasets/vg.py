...
class vg(imdb):
    def __init__(self, version, image_set, ):
        ...
        if os.path.exists(os.path.join(self._data_path,
                                       "vg_image_index_{}.p".format(
                                           self._image_set))):
            with open(os.path.join(self._data_path,
                                   "vg_image_index_{}.p".format(
                                       self._image_set)), 'rb') as fp:
                self._image_index = pickle.load(fp)
            load_index_from_file = True

        load_id_from_file = False
        if os.path.exists(os.path.join(self._data_path,
                                       "vg_id_to_dir_{}.p".format(
                                           self._image_set))):
            with open(os.path.join(self._data_path,
                                   "vg_id_to_dir_{}.p".format(self._image_set)),
                      'rb') as fp:
            ...
            if os.path.exists(filename):
                ...
                for obj in tree.findall('object'):
                    ...
                    if obj_name in self._class_to_ind:
                        # We have to actually load and check these to make 
                        # sure they have
    ...
    def _load_vg_annotation(self, index):
        ...
        for obj in objs:
            ...
            if obj_name in self._class_to_ind:
                ...
                # If bboxes are not positive, just give whole image coords (
    ...
    def _write_voc_results_file(self, classes, all_boxes, output_dir):
        for cls_ind, cls in enumerate(classes):
            ...
            filename = self._get_vg_results_file_template(output_dir).format(
                cls)
    ...
    def _do_python_eval(self, output_dir, pickle=True, eval_attributes=False):
        ...
        print('--------------------------------------------------------------')
        print(
            'Results computed with the **unofficial** PASCAL VOC Python eval '
            'code.')
        print('--------------------------------------------------------------')
