...
class AffineGridGenV2(Module):
    ...
    def forward(self, input1):
        ...
        output = torch.bmm(self.batchgrid.view(-1, self.height * self.width, 3),
                           torch.transpose(input1, 1, 2)).view(-1,
                                                               self.height,
                                                               self.width,
                                                               2)
...
class Depth3DGridGen(Module):
    ...
    def forward(self, depth, trans0, trans1, rotate):
        ...
        x = self.batchgrid3d[:, :, :, 0:1] * depth + trans0.view(-1, 1, 1,
                                                                 1).repeat(1,
                                                                           self.height,
                                                                           self.width,
                                                                           1)

        y = self.batchgrid3d[:, :, :, 1:2] * depth + trans1.view(-1, 1, 1,
                                                                 1).repeat(1,
                                                                           self.height,
                                                                           self.width,
                                                                           1)
