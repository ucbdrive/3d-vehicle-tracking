...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    for i in range(num_gt):
        ...
        if ((gt_anno["occluded"][i] > MAX_OCCLUSION[difficulty])
            ...
            # if gt_anno["difficulty"][i] > difficulty or gt_anno[
            # "difficulty"][i] == -1:
            ignore = True
...
def get_split_parts(num, num_part):
    ...
    if remain_num == 0:
        ...
        return [same_part] * num_ + [remain_num]
...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=500):
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            ...
            rots = np.concatenate([a["orientation"] for a in gt_annos_part], 0)
            ...
            rots = np.concatenate([a["orientation"] for a in dt_annos_part], 0)
...
def eval_class(gt_annos,
    ...
    """
    assert len(gt_annos) == len(dt_annos), "GT: {} DT: {}".format(len(gt_annos),
                                                                  len(dt_annos))
    num_examples = len(gt_annos)
    split_parts = get_split_parts(num_examples, num_parts)

    # This is exchangable for two annotations, will transpose the N,M
...
def get_official_eval_result(gt_annos, dt_annos, current_classes):
    ...
    overlap_0_5 = np.array([[0.5, 0.5, 0.5, 0.7, 0.5],
                            [0.5, 0.5, 0.5, 0.7, 0.5],
                            [0.5, 0.5, 0.5, 0.7, 0.5]])
    overlap_0_25 = np.array([[0.25, 0.5, 0.5, 0.7, 0.5],
                             [0.25, 0.5, 0.5, 0.7, 0.5],
                             [0.25, 0.5, 0.5, 0.7, 0.5]])
    min_overlaps = np.stack([overlap_0_7, overlap_0_5, overlap_0_25],
                            axis=0)  # [2, 3, 5]
    class_to_name = {
        0: 'Car',
        1: 'Pedestrian',
